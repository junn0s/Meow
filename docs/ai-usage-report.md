...
> **검증 상태:** 2026-07-15 최종 통합본에서 `npm run typecheck`와 `npm run build`가 통과했습니다. Vite 6.4.3이 30개 모듈을 처리해 `dist/`를 생성했고, Playwright 실브라우저 콘솔은 error 0, warning 0이었습니다. 수동 동선과 디버그 API로 튜토리얼, 첫 손님 전체 흐름, 저장 복원, 10단계 효과, 클리어와 새 게임 초기화도 확인했습니다.

GitHub Pages 실제 URL 배포와 디버그 보조 없이 처음부터 끝까지 수행한 5~8분 자연 플레이 측정은 아직 하지 않았습니다. 완료한 검증과 남은 검증을 아래 표에서 구분하며, 실행하지 않은 항목을 성공으로 기록하지 않습니다.
...
| GitHub Pages | 실제 배포 URL | 아직 배포하지 않음 | 미검증 |
...
- 게임의 재미, 난이도, 5~8분 경제 밸런스는 자동 검사만으로 보장할 수 없으며 실제 플레이테스트가 필요합니다.
- 실제 GitHub Pages 배포와 디버그 보조 없는 5~8분 자연 플레이는 남은 검증이며, 완료 후 결과를 추가해야 합니다.
